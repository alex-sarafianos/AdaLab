...
The model space shoud be able to include domain theory. In the following, we give several examples of such knowledge.
...
As stated in the above examples, regulation between genes can be either an activation or an inhibition. However, the individual expression of each regulator is not necessarily what matters the most in the end when trying to simulate the target gene's expression level. Indeed, some aggregation of activators and inhibitors could be more adequate information to infer the gene's expression level. Thus, including the activator and inhibitor roles for regulation dependencies could allow to define aggregation-oriented regulation rules, e.g.:
